...
int main(void) 
{
	...
	//--------JANELA--------
	GLFWwindow * janela = criarJanela(1024, 768, "Exemplo Interpolacao", false); //inicializa as bibliotecas GLAD e GLFW. Cria-se o contexto do opengl e janela
	...
	cout << largJan << " - " << altJan << endl;
	Matrix4f matProj = getMatProj(largJan, altJan, vFOV, zNear, zFar);

	cout << matProj << endl << endl;

	//--------Modelos 3D--------
	Objeto3D obj;
	vector<Vector3f> coordVertices;
	vector<Vector4f> corVertices;
	coordVertices.push_back(Vector3f(-0.5f, -0.5f, -3.0f));
	coordVertices.push_back(Vector3f(0.5f, -0.5f, -3.0f));
	coordVertices.push_back(Vector3f(0.0f, 0.5f, -3.0f));
	corVertices.push_back(Vector4f(1.0f, 0.0f, 0.0f, 1.0f));
	corVertices.push_back(Vector4f(0.0f, 1.0f, 0.0f, 1.0f));
	corVertices.push_back(Vector4f(0.0f, 0.0f, 1.0f, 1.0f));
	criarObjPosCor(obj, "triangulo", coordVertices, corVertices);
	...
	while (!glfwWindowShouldClose(janela) && glfwGetKey(janela, GLFW_KEY_ESCAPE) != GLFW_PRESS) //a cada iteração, um quadro é renderizado!
	{
		...
		GLuint uniformRef = glGetUniformLocation(programaShader, "mat");
