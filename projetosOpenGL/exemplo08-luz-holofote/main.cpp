...
struct Cena {
	...
	Camera camera;
	LuzHolofote luz;
	Vector3f posInicialLuz = Vector3f(8.4f, 7.3f, 0.0f);
...
int main(void) 
{
	...
	//--------JANELA--------
	GLFWwindow * janela = criarJanela(1024, 768, "Proj_OpenGL_Luz_Holofote", false); //inicializa as bibliotecas GLAD e GLFW. Cria-se o contexto do opengl e janela
	...
	//--------Modelos 3D--------
	string nomeArq = "../Objetos3D/lista_objetos_exemplo08.txt";
	...
	cena.luz.direcao = Vector3f(0.0f, -1.0f, 0.0f); //ao longo do eixo Y negativo
    cena.luz.anguloAbertura = graus2rad(25.0f); //25 graus
	cena.luz.kl = 0.15f; 
    cena.luz.kq = 0.05f;
	...
	while (!glfwWindowShouldClose(janela) && glfwGetKey(janela, GLFW_KEY_ESCAPE) != GLFW_PRESS) //a cada iteração, um quadro é renderizado!
	{
		...
		for (size_t i = 0; i < cena.objetos.size(); ++i)
		{
			...
			if(tipo == SHADER_MATERIAL_ILUM || tipo == SHADER_TEX_ILUM)
			{
				...
				glUniform1f(glGetUniformLocation(programa, "luz.anguloAbertura"), cena.luz.anguloAbertura);
...
void renderizarGUI()
{
	...
	ImGui::Begin("Configurações", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove);
	{
		...
		ImGui::NewLine();
		ImGui::Spacing();
		ImGui::Text("Definir ângulo de abertura");
		ImGui::SliderAngle("ângulo", &cena.luz.anguloAbertura, 1, 90);
		ImGui::NewLine();
