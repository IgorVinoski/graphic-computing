...
int main(void) 
{
	...
	//--------JANELA--------
	GLFWwindow * janela = criarJanela(1024, 768, "Exemplo Textura", true); //inicializa as bibliotecas GLAD e GLFW. Cria-se o contexto do opengl e janela
	...
	vector<Vector2f> coordTextura;
	coordVertices.push_back(Vector3f(-0.5f, -0.5f, -3.0f));
	coordVertices.push_back(Vector3f(0.5f, -0.5f, -3.0f));
	coordVertices.push_back(Vector3f(0.0f, 0.5f, -3.0f));
	coordTextura.push_back(Vector2f(0.0f, 0.0f));
	coordTextura.push_back(Vector2f(1.0f, 0.0f));
	coordTextura.push_back(Vector2f(0.5f, 1.0f));
	criarObjPosTex(obj, "maca", coordVertices, coordTextura, "../Objetos3D/maca.jpg");
