...
struct Cena {
	...
	Camera camera;
	Vector3f posInicialCamera = Vector3f(0, 2.3, 4.5);
	LuzDirecional luz;
	float angRotLuz = 0;
	...
	int objtoGUISel = 0;
	ShaderTipo sombreamentoSel = SHADER_FLAT;
...
int main(void) 
{
	...
	//--------JANELA--------
	GLFWwindow * janela = criarJanela(1024, 768, "Proj_OpenGL_Sombreamento-Phong-Gouraud-Flat", false); //inicializa as bibliotecas GLAD e GLFW. Cria-se o contexto do opengl e janela
	...
	shaders[SHADER_FLAT] = criarPrograma ("shaders/flatVtx.glsl", "shaders/flatFrag.glsl");
	shaders[SHADER_GOURAUD] = criarPrograma ("shaders/gouraudVtx.glsl", "shaders/gouraudFrag.glsl");
	shaders[SHADER_PHONG] = criarPrograma ("shaders/phongVtx.glsl", "shaders/phongFrag.glsl");
	...
	//--------Modelos 3D--------
	string nomeArq = "../Objetos3D/lista_objetos_exemplo09.txt";
	...
	while (!glfwWindowShouldClose(janela) && glfwGetKey(janela, GLFW_KEY_ESCAPE) != GLFW_PRESS) //a cada iteração, um quadro é renderizado!
	{
		...
		for (size_t i = 0; i < cena.objetos.size(); ++i)
		{
			...
			GLuint programa = shaders[cena.sombreamentoSel];
...
void keyEvent(GLFWwindow *win, int key, int scancode, int action, int mods)
{
	...
	if (key == GLFW_KEY_F12 && action == GLFW_PRESS)
	{
		...
		capturarTela("../captura/", largura, altura);
	}
...
void renderizarGUI()
{
	...
	ImGui::SetNextWindowSize(ImVec2(310, 750), ImGuiCond_Once);
	...
	ImGui::Begin("Configurações", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove);
	{
		...
		TextColored(ImVec4(1,1,0,1), "Tipo de sombreamento");
		const char* sombreamentos[] = {
			"Flat",
			"Gouraud",
			"Phong"
		};

		static int sombSel = 0;
		if( ImGui::Combo("##sombreamentoTipo", &sombSel, sombreamentos, IM_ARRAYSIZE(sombreamentos)) )
		{
			cena.sombreamentoSel = static_cast<ShaderTipo>(sombSel);
		}
